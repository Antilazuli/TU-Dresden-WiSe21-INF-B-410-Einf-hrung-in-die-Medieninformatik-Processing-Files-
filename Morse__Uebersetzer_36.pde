...
PFont font;
int switchCase, entroCount, len = 2, lenIter = 2, usedChars = 0, bridge;
float entropy, entropyAlt, medianLen, redundancy;
...
void draw(){  
  ...
  for (int i = 0;i < input.length(); i++){
   ...
   for(int i = 0; i < countArrayPI.length; i++){
     if(countArrayPI[i] != 0){
       ...
       //println("pi(" + (float)countArrayPI[i] + ") * Ni(" + wordLen[usedChars] + ") = " + countArrayPI[i] * wordLen[usedChars]);
       //medianLen += (float)countArrayPI[i] * wordLen[usedChars];
       //usedChars++;
       
       usedChars++;
       medianLen += (float)countArrayPI[i] * usedChars;
       bridge = i;
       
     }
   }
   
   medianLen = medianLen - countArrayPI[bridge] * usedChars + countArrayPI[bridge] * (usedChars - 1);
  ...
  if(input.length() < 1){
   ...
   text("L: " + Math.round(Math.pow(10.0, medianLen) * entropyAlt) / Math.pow(10.0, roundSetting) + " Bits", width * 0.5, height * 0.5);
   text("R: " + /*Math.round(Math.pow(10.0, redundancy) * entropyAlt) / Math.pow(10.0, roundSetting) +*/ (medianLen - entropyAlt) + " Bits", width * 0.5, height * 0.95);
  }
  //parameter reset
  morseCode = "";
  entroCount = 0;
